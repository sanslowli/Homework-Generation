...
- **DailyHomework = 학생 선택제 3/5/10장**(3=현행2+지난1 · 5=현행3+지난2 · 10=현행6+지난4, 부족분 자동 보충) — **0707 San 결정, 웹앱 기준.** 데일리 시작 시 개수 선택 화면(버튼 3개)이 먼저 뜸. (이력: 0608 '3장 고정' → 0701 '10장 고정'[쉐도잉 안 함·공 수 부족 체감] → 0707 현장에서 10장 과다 체감[규원 관찰] → 선택제 전환. ※ 레거시 Streamlit `app.py`는 아직 현행2+지난1=3장일 수 있음 — 학생 대면 정본은 웹앱, 현황은 `kusukmap-webapp/changelogs`.) **구간 안배(하이브리드 A): 각 풀에서 구간별 최소 1개 보장 후 나머지 약점순** — 한 구간 쏠림(구간1만 나오던 고은석 603S) 방지.
