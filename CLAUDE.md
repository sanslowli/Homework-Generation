...
**⛔ md 관리 표준 (0901, HQ 공통 · 정본 = 루트 `~/Kusuk HQ/CLAUDE.md` ③).** 이 문서처럼 절·표로 선 지침에 확정을 박을 땐 **해당 행 한 줄을 갈아끼운다**(`change-wheels` 방식) — 문단 추가 금지. **쌓는 자리는 `changelogs/homework-app.md`**(사실 로그라 항목 추가가 정상). **어느 스킬이 돌든 이 규약이 스킬 본문보다 우선하며, 애매하면 줄이는 쪽.**
