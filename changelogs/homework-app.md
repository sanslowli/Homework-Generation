...
- **수강생 효과**: 스캔 후 담기 정보가 조용히 사라지거나 어긋나는 사고 차단.

## 🚑 파이프라인 — 구글 429(쿼터) 재시도 + TTS 실패 정직화 (2026-08-01)
- **변경**: ①`sync_notion.with_retry` 재시도 대상에 **429 추가**(종전 5xx만 — 429는 4xx라 즉시 raise였음) + 횟수 3→5회·기본 지연 5→10초(10·20·40·80초 = 분당 쿼터 창을 넘김) ②`generate_tts.py`에 같은 규약의 `with_retry` 신설(시트 열기·읽기에 적용) ③**TTS 실패 시 exit 1**(종전엔 전 문장 실패해도 exit 0 = 워크플로 초록) ④워크플로의 커밋 스텝을 `if: always()`로 — 실패해도 성공분 mp3는 커밋(부분 성과 보존).
