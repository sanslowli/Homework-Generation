        ...
        (function(){{
          ...
          function pickEnglishVoice() {{
            try {{
              ...
              // iOS Samantha, macOS Samantha, Google US English 등 영어 우선
              ...
              return voices.find(function(v){{ return v.lang && v.lang.toLowerCase().indexOf('en') === 0; }});
            }} catch (e) {{ return null; }}
          }}
          function speak() {{
          ...
          // 일부 브라우저는 voices가 비동기 로딩됨 → 한 번 미리 trigger
          if ('speechSynthesis' in window) {{
...
<p style="font-size:17px;font-weight:700;margin-top:32px;margin-bottom:10px;">정답 입력</p>

피칭 연습 중 정답 문장이 기억나지 않을 때마다 교재를 펼치거나 Notion을 열어 확인하기 번거로우신가요? 메인 화면 왼쪽 사이드바의 '정답 입력' 버튼을 통해 어려운 손그림의 정답을 미리 입력해두면, 피칭 시 화면을 꾹 누르고 있는 동안 정답 문장이 표시됩니다.
