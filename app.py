...
def render_answer_reveal(answer_text, button_label="🔒 정답 보기 (꾹 누르기)"):
    ...
    # 버튼(45) + margin(10) + answer 박스 padding 30 + line 23px(14*1.6) * n + 버퍼 12
    total_h = 45 + 10 + 30 + est_lines * 23 + 12
...
if st.session_state['mode'] == 'setup':
    ...
    # 상단 돌아가기
    if st.button("⬅️ 처음으로", key="help_back_top"):
        st.session_state['mode'] = 'setup'
        st.rerun()
...
Syntax Pitching™은 **영어 문장을 "치는" 타격 연습 도구**예요.

매일 정해진 수의 그림(4컷 만화 같은)이 한 장씩 나오면, 수강생이 그 장면에 맞는 영어 문장을 **소리 내어** 말합니다(이게 "피칭"). 잘 말했는지 못했는지 스스로 판정하면, 앱이 기록을 쌓아서 **타율**을 내줘요. 매일 배팅 케이지에 들어가는 야구 선수처럼, 하루 10장씩 치는 감각 운동입니다.
...
**수강생 선택**
개인용 URL(`?student=이름`)로 바로 들어오거나, 왼쪽 사이드바에서 본인 이름 선택.

**사이드바 메뉴**

- **챕터 선택** — 여러 개 고를 수 있음
- **타율 필터** — "60% 이하"처럼 약한 것만 집중 훈련
- **훈련 시작 (Start)** — 선택한 챕터로 바로 연습
- **피칭 기록 보기** — 과거 성적 조회
- **📝 정답 입력** — 그림에 대응하는 영어 문장 등록

---

## 🌅 Daily Homework — 매일 10장

메인 화면에서 **"오늘의 Daily Homework 시작"** 버튼을 누르면 자동으로 오늘의 10장이 뽑혀요.

- 🗂️ 현행 챕터 **6장** + ✔️ 지난 챕터 **4장**
- 한쪽이 부족하면 반대쪽에서 자동 보충 (항상 총 10장 보장)
- **아직 5번 미만 쳐본 "신입" 그림**이 우선 → 그다음 **타율 낮은 그림** 우선

> 신입사원 적응 기간처럼, 새 그림에겐 먼저 "기회"를 주는 구조예요.

완료하면 **오늘 날짜 · 출석 달력 · 챕터별 타율**이 박힌 카톡 인증 이미지가 자동 생성. 꾹 눌러 복사해서 단톡방에 붙여넣으면 끝.
...
1. **큰 소리로** 영어 문장을 말한다
2. 잘 말했으면 🙆 **통과**, 막혔으면 🙅 **미통과** 직접 판정
3. 다음 그림으로 자동 이동

말하다 막혔을 때는 아래의 **🔒 정답 보기 (꾹 누르기)** 버튼을 누르고 있는 동안 정답이 떠요. 손을 떼면 사라짐. 커닝페이퍼 훔쳐보듯, 필요할 때만 살짝.

**이전 취소 (Undo)** 버튼으로 바로 앞 결과는 되돌릴 수 있어요.

---

## 📝 정답 등록은 어떻게?

사이드바에서 챕터 고르고 **📝 정답 입력** 메뉴 진입. 그림마다 텍스트 입력칸이 뜹니다.

- 그림 한 장이 **2~4칸**으로 나뉜 경우: 각 칸의 영어 문장을 **엔터로 줄바꿈**해서 이어 적기
- 💾 저장 = 즉시 반영, 나중에 수정 가능
- 한 번 등록하면 훈련 / 결과 / 기록 화면 어디서든 꾹 누르기로 확인

정답을 한 번 제대로 등록해두면, 이후엔 훈련할 때마다 참고 자료로 쓸 수 있어요.

---

## 📊 "타율" 이 뭐예요?

같은 그림을 여러 번 쳤을 때 **최근 5회 중 O 비율**입니다.
...
- **5회 미만** 쳐본 그림은 "신입"으로 취급 → 데이터 충분히 쌓일 때까지 우선 출제
- 약한 그림일수록 **다음번 출제될 확률이 자동으로 높아짐**

> 야구에서 3할 타자가 "잘 친다"는 소리를 듣는다면, 여기선 80% 넘기면 일단 합격선.

---

## 💡 자주 쓰는 팁

- **매일 10장 꾸준히**가 최고. 몰아서 하루 50장 치는 건 효율 낮아요 — 근육은 매일 자극해야 붙어요
- **틀린 구간 반복** 버튼으로 세션 끝난 뒤 약점만 골라 재도전
- **타율 필터 40% 이하** 모드로 집중 훈련
- 잊지 말고 **Daily Homework 카톡 인증** — 출석 기록이 달력에 쌓입니다

---

## 🎓 선생님이시라면

URL에 `?teacher=1` 을 붙여서 접속하면 **Teacher Dashboard**가 열려요. 수강생 선택해서:

- 챕터별 집계 (평균 타율, 약점 그림 수)
- 이미지별 상세
- 약점 TOP 10
- 출석 현황 달력

이렇게 네 개 탭을 한 화면에서 확인할 수 있습니다.
...
""")
    ...
    st.markdown("---")
    if st.button("⬅️ 처음으로 돌아가기", use_container_width=True, key="help_back_bottom"):
        st.session_state['mode'] = 'setup'
        st.rerun()
